...
#define FT_REG_VERSION_ID	0xA8 //v_id register
...
static int ft5x06_fw_upgrade(struct device *dev, bool force)
{
	...
	FT_STORE_TS_INFO(data->ts_info, data->family_id, data->pdata->name,
			...
			data->pdata->firmware->ft_fw_name, data->fw_ver[0],
			data->fw_ver[1], data->fw_ver[2]);
rel_fw:
	release_firmware(fw);
	return rc;
					...
					struct ft5x06_ts_data *data)
{
		...
		if (data->vendor_id == firmware->vendor_id) {
			if (client->dev.of_node) {
				if(data->chip_id == firmware->chip_id) {
					dev_info(&client->dev, "Found Firmware!\n");
					break;
				} else
					continue;
			} else {
	...
	if(client->dev.of_node) {
		...
		if (firmware->ft_fw_name) {
                	len = strlen(firmware->ft_fw_name);
               		if (len > FT_FW_NAME_MAX_LEN - 1) {
                        	dev_err(&client->dev, "Invalid firmware name\n");
                        	return -EINVAL;
               		}
                	dev_info(&client->dev, "Firmware Loaded = %s\n", firmware->ft_fw_name);
                	strlcpy(data->fw_name, firmware->ft_fw_name, len + 1);
		}
        }
			   ...
			   const struct i2c_device_id *id)
{
	...
	/* Detect vendor and grab our firmware */
	err = ft5x06_load_firmware(pdata, data);
	if (err)
		dev_err(&client->dev, "Firmware cannot be loaded");
